...
class TestAbstractOrchestrator(unittest.TestCase):
    ...
    def test_is_initialized_raises_exception_when_mapper_manager_is_not_defined(
        self,
    ):
        with self.assertRaises(RuntimeError):
            self.orchestrator.is_initialized()

    def test_is_initialized_raises_exception_when_input_manager_is_not_defined(self):
        self.orchestrator.mapper_manager = MagicMock()
        with self.assertRaises(RuntimeError):
            self.orchestrator.is_initialized()

    def test_is_initialized_raises_exception_when_transformer_manager_is_not_defined(self):
        self.orchestrator.mapper_manager = MagicMock()
        self.orchestrator.input_manager = MagicMock()
        with self.assertRaises(RuntimeError):
            self.orchestrator.is_initialized()

    def test_is_initialized_raises_exception_when_output_manager_is_not_defined(self):
        self.orchestrator.mapper_manager = MagicMock()
        self.orchestrator.input_manager = MagicMock()
    ...
    def test_set_transformer_manager_adds_transformer_manager_correctly(self):
        ...
        transformer_manager_mock.get_id.return_value = "test_transformer_manager"
   
        self.orchestrator.set_transformer_manager(transformer_manager_mock)
   
        self.assertIn("test_transformer_manager", self.orchestrator.transformer_manager)
        self.assertEqual(
